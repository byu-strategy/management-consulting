...
.firm-type-badge.boutiquestrategy {
  background: #374151;
  color: white;
}

.firm-type-badge.economicconsulting {
  background: #7c3aed;
  color: white;
}

.firm-type-badge.restructuring {
  background: #dc2626;
  color: white;
}

.firm-type-badge.lifesciences {
  background: #059669;
  color: white;
}

.firm-type-badge.governmentdefense {
  background: #0369a1;
  color: white;
}

.firm-type-badge.hrtalent {
  background: #d97706;
  color: white;
}

.firm-type-badge.healthcare {
  background: #0891b2;
  color: white;
}

.firm-type-badge.technologydigital {
  background: #4f46e5;
  color: white;
}

.firm-type-badge.petransactionadvisory {
  background: #be123c;
  color: white;
}

.firm-type-badge.regionalniche {
  background: #6b7280;
  color: white;
}
...
/* =============================================================================
   UNIFIED HOVER TOOLTIPS - Alumni, Offices, Comp
   ============================================================================= */

/* Base positioning for all tooltip parents */
.hero-stat.alumni-stat,
.hero-stat.offices-stat,
.hero-stat.comp-stat {
  position: relative;
  cursor: default;
}

/* Base tooltip styling - shared by all */
.alumni-tooltip,
.offices-tooltip,
.comp-tooltip {
  display: none;
...
.hero-stat.alumni-stat:hover .alumni-tooltip,
.hero-stat.offices-stat:hover .offices-tooltip,
.hero-stat.comp-stat:hover .comp-tooltip {
  display: block;
}

/* Alumni tooltip specific */
.alumni-tooltip {
  white-space: nowrap;
}

.alumni-tooltip-header-row {
  display: grid;
  grid-template-columns: 55px 50px 50px;
  font-weight: 700;
  text-transform: uppercase;
  font-size: 0.55rem;
  letter-spacing: 0.04em;
...
.alumni-tooltip-header-row span:nth-child(2),
.alumni-tooltip-header-row span:nth-child(3) {
  text-align: center;
}
...
.alumni-tooltip-row .label {
  color: #94a3b8;
  font-size: 0.7rem;
}

.alumni-tooltip-row .value {
  font-weight: 600;
  font-size: 0.75rem;
  font-family: ui-monospace, SFMono-Regular, "SF Mono", Menlo, Consolas, monospace;
  text-align: center;
...
/* Offices tooltip specific */
.offices-tooltip {
  max-width: 300px;
  width: max-content;
}

.offices-tooltip-grid {
  display: grid;
  grid-template-columns: repeat(2, 1fr);
  gap: 0.2rem 1rem;
}

.offices-tooltip-item {
  font-size: 0.7rem;
  color: #e2e8f0;
  padding: 0.1rem 0;
}

/* Tooltip links - shared style */
.alumni-tooltip-link,
.alumni-tooltip-link:link,
.alumni-tooltip-link:visited {
...
.alumni-tooltip-link:hover {
  color: #93c5fd !important;
}

/* BYU Alumni Links Row in Drawer */
.byu-alumni-links {
  display: flex;
  gap: 0.5rem;
  flex-wrap: wrap;
  ...
  <div class="filter-row filter-row-filters">
    <div class="filter-group">
      ...
      <select id="filter-type">
        ...
        <option value="Boutique Strategy">Boutique Strategy</option>
        <option value="Economic Consulting">Economic</option>
        <option value="Restructuring">Restructuring</option>
        <option value="Life Sciences">Life Sciences</option>
        <option value="Government/Defense">Gov/Defense</option>
        <option value="HR/Talent">HR/Talent</option>
        <option value="Healthcare">Healthcare</option>
        <option value="Technology/Digital">Tech/Digital</option>
        <option value="PE/Transaction Advisory">PE/M&A</option>
        <option value="Regional/Niche">Regional/Niche</option>
      </select>
    </div>
...
<div id="offices-popover" class="offices-popover" onmouseleave="closeOfficesPopover()">
  ...
  return `
    <div class="firm-card"
      ...
      <div class="hero-stats">
        <!-- 1. BYU + Marriott Alumni (Combined Total) -->
        <div class="hero-stat pipeline-stat alumni-stat ${pipelineStatClass}">
          <div class="hero-stat-header">${icons.graduationCap} BYU Alumni</div>
          ${(firm.byu?.alumni_current || firm.byu?.marriott_alumni_current) ? `
            <div class="hero-stat-value">
              <span class="offices-count-link">${(firm.byu?.alumni_current || 0) + (firm.byu?.marriott_alumni_current || 0)}</span>
            </div>
            <div class="hero-stat-detail">${(firm.byu?.alumni_all_time || 0) + (firm.byu?.marriott_alumni_all_time || 0)} all-time</div>
            <div class="alumni-tooltip">
              <div class="alumni-tooltip-header-row"><span></span><span>Current</span><span>Past</span></div>
              <div class="alumni-tooltip-row">
                <span class="label">BYU</span>
                <span class="value">${firm.links?.byu_current ? `<a href="${firm.links.byu_current}" target="_blank" class="alumni-tooltip-link">${firm.byu?.alumni_current || 0}</a>` : (firm.byu?.alumni_current || 0)}</span>
                <span class="value">${firm.links?.byu_past ? `<a href="${firm.links.byu_past}" target="_blank" class="alumni-tooltip-link">${(firm.byu?.alumni_all_time || 0) - (firm.byu?.alumni_current || 0)}</a>` : ((firm.byu?.alumni_all_time || 0) - (firm.byu?.alumni_current || 0))}</span>
              </div>
              <div class="alumni-tooltip-row">
                <span class="label">Marriott</span>
                <span class="value">${firm.links?.marriott_current ? `<a href="${firm.links.marriott_current}" target="_blank" class="alumni-tooltip-link">${firm.byu?.marriott_alumni_current || 0}</a>` : (firm.byu?.marriott_alumni_current || 0)}</span>
                <span class="value">${firm.links?.marriott_past ? `<a href="${firm.links.marriott_past}" target="_blank" class="alumni-tooltip-link">${(firm.byu?.marriott_alumni_all_time || 0) - (firm.byu?.marriott_alumni_current || 0)}</a>` : ((firm.byu?.marriott_alumni_all_time || 0) - (firm.byu?.marriott_alumni_current || 0))}</span>
              </div>
            </div>
          ` : '<div class="hero-stat-value">—</div>'}
        </div>

        <!-- 2. Offices (Hover Tooltip) -->
        <div class="hero-stat offices-stat">
          <div class="hero-stat-header">${icons.mapPin} Offices</div>
          ${offices.length > 0 ? `
            <div class="hero-stat-value">
              <span class="offices-count-link">${offices.length}</span>
            </div>
            <div class="hero-stat-detail">${displayOffices.join(' · ')}${moreOfficesCount > 0 ? ` +${moreOfficesCount}` : ''}</div>
            <div class="offices-tooltip">
              <div class="offices-tooltip-grid">
                ${offices.map(office => `<div class="offices-tooltip-item">${office}</div>`).join('')}
              </div>
            </div>
          ` : '<div class="hero-stat-value">—</div>'}
        </div>
      ...
      <div class="firm-details">
        ...
        <!-- 1. BYU TIPS -->
        ${firm.byu?.tip || firm.email_format || firm.links?.byu_current ? `
        <div class="interview-section byu-tips-section">
          ...
          ` : ''}
          ${firm.links?.byu_current || firm.links?.byu_past || firm.links?.marriott_current || firm.links?.marriott_past ? `
          <div class="byu-alumni-links">
            ${firm.links?.byu_current ? `<a href="${firm.links.byu_current}" target="_blank">BYU Current</a>` : ''}
            ${firm.links?.byu_past ? `<a href="${firm.links.byu_past}" target="_blank">BYU Past</a>` : ''}
            ${firm.links?.marriott_current ? `<a href="${firm.links.marriott_current}" target="_blank">Marriott Current</a>` : ''}
            ${firm.links?.marriott_past ? `<a href="${firm.links.marriott_past}" target="_blank">Marriott Past</a>` : ''}
          </div>
          ` : ''}
        </div>
        ` : ''}
...
- **Boutique Strategy** — Global strategy firms outside MBB/Big 4 (Kearney, Oliver Wyman, L.E.K., etc.)
- **Economic Consulting** — Expert testimony and litigation support (Analysis Group, NERA, Brattle, etc.)
- **Restructuring** — Turnaround and crisis management (Alvarez & Marsal, AlixPartners, FTI)
- **Life Sciences** — Pharma and healthcare commercial strategy (ZS, Putnam, DeciBio, ClearView)
- **Government/Defense** — Federal consulting and public sector (Booz Allen, Guidehouse)
- **HR/Talent** — Organizational and talent strategy (Mercer, Korn Ferry)
- **Healthcare** — Provider and payer consulting (Huron, Chartis, Leavitt Partners)
- **Technology/Digital** — Digital transformation and implementation (Accenture, Slalom, West Monroe)
- **PE/Transaction Advisory** — Due diligence and deal support (Stax, Palm Tree)
- **Regional/Niche** — Specialized or regional firms (Cicero, Bridgespan, etc.)
