...
Scored by the professor. This rubric evaluates the deck as a standalone artifact. Would a busy executive actually read this and take action?
...
| Rating | Description |
|--------|-------------|
| 7 | Reading only the action titles tells the complete story. Clear arc from insight through opportunity to ask. Every slide earns its place. Reads as a professional document with no assignment artifacts, and includes your name and contact information. |
| 6 | Strong narrative arc with action titles throughout. One slide could be tighter or better sequenced, but the story holds. No assignment artifacts (no "P1/P2" references, no assignment headers). |
| 5 | Clear point of view and logical flow. A couple of titles are labels rather than conclusions ("Market Overview" instead of "Market is shifting toward X"). May contain minor assignment artifacts. |
...
| 3 | Follows the required slide structure but the narrative doesn't build. Reads more like a checklist (company overview, data, opportunity, ask) than a story. Assignment structure is visible, with section headers matching the template rather than the story. |
...
Any slide that cites a number must include a traceable source: a named report, filing, earnings call, survey, or database. "Web research," "various sources," "analyst estimates" (without naming the analyst), and "proprietary analysis" (without showing assumptions) are not traceable sources. Unverifiable sourcing on key claims caps the Analytical Depth score at 5, regardless of how strong the insight is.
:::

| Rating | Description |
|--------|-------------|
| 7 | Non-obvious insight backed by specific, sourced data. Quantified impact is credible and well-bounded. Sources are traceable (named reports, filings, or earnings calls with dates). Someone at the company would learn something they didn't already know. |
| 6 | Strong insight with solid evidence. Quantification is reasonable; one assumption could be better justified. Sources are traceable on all key claims. |
| 5 | Good analysis with a clear point of view. Evidence supports the argument but relies on one or two unsupported leaps or round-number estimates. Some sources may be vague or unverifiable. |
| 4 | Adequate analysis, but the opportunity is plausible with evidence that is thin in places. "So what" is stated but not fully earned by the data. Numbers may be internally inconsistent across slides. |
| 3 | Surface-level research. Uses easily available facts (company Wikipedia, recent headlines) without digging into financials, competitive dynamics, or trends. Sources listed in appendix but not on slides. |
| 2 | Generic observations that could apply to any company in the industry. Little or no quantification. Data labeled "*illustrative" without supporting analysis. |
...
| Rating | Description |
|--------|-------------|
| 7 | Polished and professional. One message per slide, clean charts, tight text, consistent formatting. Passes the squint test: key messages are clear from a distance. Consistent visual logic (color, layout) throughout. |
| 6 | Clean and readable throughout. One slide is slightly dense or one chart could be clearer, but overall professional. Formatting is consistent. |
...
1. **What I Noticed** (2-4 slides): Your outside-in insight (refined from P1). What is happening at this company that they should be paying attention to?
2. **The Opportunity** (1-3 slides): Your point of view (refined from P2). What should they do about it, and why now?
3. **What's at Stake** (1-2 slides): Quantified impact. What happens if they act vs. don't act?
4. **Proposed Discussion** (1 slide): Two or three specific questions you'd want to explore together, and what you'd hope to learn. This is your pitch for a 20-minute conversation, so make the questions specific enough that the reader thinks "I'd actually want to talk about that."

**Appendix**: Supporting detail (data tables, methodology notes, additional analysis). Keep the main deck tight and narrative-driven. A busy exec will skim the main slides; the appendix shows you've done the work if they want to dig deeper.
