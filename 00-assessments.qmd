...
| Capstone | Averaged → presentation score (50%) | 50 presentation + 50 deck quality |
...
[Submit Presentation Feedback](https://docs.google.com/forms/d/e/1FAIpQLSccVuQm1vhbx2-6-AC6vDEKqXnkLnaVVIY3RAzCN7JWk6oWMw/viewform) — Complete this form for each pod member after their presentation. The same form is used for P1, P2, and Capstone.
:::

### Feedback Form {#presentation-rubric}

This form is used for all three projects. For P1 and P2, ratings provide developmental feedback. For Capstone, ratings are averaged to determine your presentation score (50 pts — half of the deck grade). The other half comes from the [Deck Quality Rubric](#deck-quality-rubric), scored by the professor.
...
**Capstone Presentation Scoring**: Average all raters' scores (peers + TA), then convert to 50-point scale:

| Dimension | Weight | Max | 4 | 3 | 2 | 1 |
|-----------|--------|-----|---|---|---|---|
| Informed | 40% | 20 | 20 | 15 | 10 | 5 |
| Compelling | 40% | 20 | 20 | 15 | 10 | 5 |
| Credible | 20% | 10 | 10 | 7.5 | 5 | 2.5 |
| **Total** | 100% | **50** | 50 | 37.5 | 25 | 12.5 |
...
|-----------|-----------|--------|
| Presentation quality ([Feedback Form Rubric](#presentation-rubric), averaged) | Peers + TA | 50 |
| Deck quality ([Deck Quality Rubric](#deck-quality-rubric)) | Professor | 50 |
| **Total** | | **100** |
| Outreach completed | Extra credit | +5 |

#### Deck Quality Rubric {#deck-quality-rubric}

Scored by the professor. Five dimensions, each on a 7-point scale, converted to 50 points using the formula below.
...
**1. Storyline & Structure (25%)** {#rubric-storyline}

*Does the deck tell a logical, answer-first story?*

| Rating | Description |
|--------|-------------|
| 7 | Clear SCR arc (Situation, Complication, Resolution) with the resolution dominating the deck. Situation and complication are tight, giving most space to the opportunity and ask. Every slide advances the argument; removing any one would weaken the deck. Reads as a standalone professional document with no assignment artifacts. Includes your name and contact information. |
| 6 | Strong narrative arc. The resolution still dominates, but the situation/complication section could be one slide tighter. No assignment artifacts ("P1/P2" references, assignment headers). |
| 5 | Clear point of view and logical flow. The balance between setup and payoff is slightly off (e.g., too much context, not enough on the opportunity). May contain minor assignment artifacts. |
| 4 | The argument is there but you have to work to follow it. Some slides feel out of order, or the connection between insight and ask is unclear. Title slide still says "P2: Point of View" or similar. |
| 3 | Follows the required slide structure but the narrative doesn't build. Reads more like a checklist (company overview, data, opportunity, ask) than a story. Assignment structure is visible, with section headers matching the template rather than the story. Includes slides that don't advance the argument. |
| 2 | Disjointed. Slides make individual points but don't connect into an argument. No clear "so what" driving the deck. May feel like two separate assignments stapled together. |
| 1 | Missing major structural components or reads as a data dump with no narrative thread. |

**2. Action Titles (20%)** {#rubric-action-titles}

*Do the slide titles tell the story on their own?*

| Rating | Description |
|--------|-------------|
| 7 | Every title is a specific, quantified sentence stating the slide's conclusion ("Costs grew 2x faster than revenue," not "Cost Analysis"). Reading only the titles in sequence tells a coherent, persuasive story. Titles use active voice, fit within two lines (~15 words), and pass the Golden Rule: nothing in the title that isn't proved in the body, nothing in the body that's irrelevant to the title. |
| 6 | All titles are action titles (conclusions, not labels). The title sequence holds together as a story. One title could be sharper, more specific, or better quantified. Golden Rule holds throughout. |
| 5 | Most titles are action titles. A couple are topic labels ("Market Overview" instead of "Market is shifting toward X"). Title sequence mostly flows but has a gap. |
| 4 | Mix of action titles and topic labels. You need the slide bodies to follow the argument. Some titles could belong to any company or any project. |
| 3 | More labels than conclusions. Titles describe processes or topics ("Our Analysis," "Financial Data") rather than state findings. If you can't write a clear action title, the slide's message isn't clear. |
| 2 | Titles are generic headers. No "so what" in any title. Titles and slide bodies feel disconnected. |
| 1 | No meaningful titles, or titles are absent/placeholder text. |

**3. Evidence & Insight (30%)** {#rubric-evidence}

*Does the evidence earn the "so what"?*

::: {.callout-important}
## Source Quality Gate

Any slide that cites a number must include a traceable source: a named report, filing, earnings call, survey, or database. "Web research," "various sources," "analyst estimates" (without naming the analyst), and "proprietary analysis" (without showing assumptions) are not traceable sources. Unverifiable sourcing on key claims caps the Evidence & Insight score at 5, regardless of how strong the insight is.
:::

| Rating | Description |
|--------|-------------|
| 7 | Non-obvious insight backed by specific, sourced data. Quantified impact is credible and well-bounded. Conclusions are pressure-tested: assumptions are stated, alternatives acknowledged, and the analysis addresses the strongest counterargument. Sources are traceable (named reports, filings, or earnings calls with dates). Someone at the company would learn something they didn't already know. |
| 6 | Strong insight with solid evidence. Quantification is reasonable; one assumption could be better justified. The "so what" is clear at every level: each slide answers "what does this mean for the decision?" Sources are traceable on all key claims. |
| 5 | Good analysis with a clear point of view. Evidence supports the argument but relies on one or two unsupported leaps or round-number estimates. Some slides present data without fully drawing out the implication. |
| 4 | Adequate analysis, but evidence is thin in places. "So what" is stated but not fully earned by the data. Numbers may be internally inconsistent across slides. Format choices feel default (e.g., bullets where a chart would be clearer, or a chart without a clear takeaway). |
| 3 | Surface-level research. Uses easily available facts (company Wikipedia, recent headlines) without digging into financials, competitive dynamics, or trends. The deck presents observations but doesn't push to insight. Sources listed in appendix but not on slides. |
| 2 | Generic observations that could apply to any company in the industry. Little or no quantification. Data labeled "illustrative" without supporting analysis. Reads as a research report, not a decision document. |
| 1 | No real analysis. Assertions without evidence, or irrelevant data that doesn't connect to the opportunity. |

**4. The Ask (10%)** {#rubric-ask}

*Are the proposed next steps worth pursuing?*

| Rating | Description |
|--------|-------------|
| 7 | The proposed next steps flow directly from the analysis. Each one references a specific finding, describes what you'd actually do, and implies a deliverable. The recipient thinks "this person could actually help me." The ask is proportional to the insight offered: you earned the right to propose work by demonstrating value first. |
| 6 | Clear, specific next steps connected to the analysis. One could be sharper or more clearly tied to a specific finding in the deck. |
| 5 | Reasonable next steps, but somewhat generic (could apply to any company in the industry) or not clearly connected to the preceding analysis. |
| 4 | The ask exists but feels formulaic. Proposed steps are broad ("explore growth opportunities") rather than earned by specific findings. |
| 3 | Vague or disconnected from the deck's argument. Feels tacked on rather than flowing from the story. |
| 2 | Perfunctory. One generic item or a request with no specificity. |
| 1 | Missing or incomprehensible. |

**5. Visual Craft (15%)** {#rubric-visual}

*Would a busy exec actually read this?*

| Rating | Description |
|--------|-------------|
| 7 | Polished and professional. One message per slide. Passes the squint test: key messages are visually clear from across the room. Consistent formatting throughout: fonts, colors, alignment, and margins. Color is used strategically to highlight key data, not decorate. Clean charts with clear takeaways, sourced in footnotes. Consistent text hierarchy (two font sizes, two points apart). No chart junk, no animations, generous white space. |
...
| 4 | Readable but unpolished. Mix of strong and weak slides, with some clean and others text-heavy or visually cluttered. May have redundant content (e.g., "Key Takeaways" sidebar that restates what's already on the slide). |
...
Each dimension is scored 1-7. The weighted average is converted to 50 points:

> **Deck Quality Score** = (Storyline x 0.25 + Titles x 0.20 + Evidence x 0.30 + Ask x 0.10 + Visual x 0.15) x 50 / 7

| Weighted Avg | Points (out of 50) |
|---|---|
| 7.0 | 50 |
| 6.0 | 43 |
| 5.0 | 36 |
