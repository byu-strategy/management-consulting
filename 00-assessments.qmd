...
| Rating | Description |
|--------|-------------|
| 7 (A) | Reading only the action titles tells the complete story. Clear SCQA arc from opening through insight to ask. Every slide earns its place — nothing to cut, nothing missing. |
| 6 (A-) | Strong narrative arc with action titles throughout. One slide could be tighter or better sequenced, but the story holds. |
| 5 (B+) | Clear point of view and logical flow. A couple of titles are labels rather than conclusions ("Market Overview" instead of "Market is shifting toward X"). |
| 4 (B) | The argument is there but you have to work to follow it. Some slides feel like they're in the wrong order, or the connection between insight and ask is unclear. |
| 3 (B-) | Follows the required slide structure but the narrative doesn't build. Reads more like a checklist (company overview → data → opportunity → ask) than a story. |
| 2 (C) | Disjointed — slides make individual points but don't connect into an argument. No clear "so what" driving the deck. |
...
| Rating | Description |
|--------|-------------|
| 7 (A) | Non-obvious insight backed by specific, sourced data. Quantified impact is credible and well-bounded. Someone at the company would learn something they didn't already know. |
| 6 (A-) | Strong insight with solid evidence. Quantification is reasonable; one assumption could be better justified or sourced. |
| 5 (B+) | Good analysis with a clear point of view. Evidence supports the argument but relies on one or two unsupported leaps or round-number estimates. |
| 4 (B) | Adequate analysis — the opportunity is plausible but the evidence is thin in places. "So what" is stated but not fully earned by the data. |
| 3 (B-) | Surface-level research. Uses easily available facts (company Wikipedia, recent headlines) without digging into financials, competitive dynamics, or trends. |
| 2 (C) | Generic observations that could apply to any company in the industry. Little or no quantification. |
| 1 (D) | No real analysis — assertions without evidence, or irrelevant data that doesn't connect to the opportunity. |

**Slide Craft (20%)** — *Would a busy exec actually read this?*

| Rating | Description |
|--------|-------------|
| 7 (A) | Polished and professional. One message per slide, clean charts, tight text, consistent formatting. Passes the squint test — key messages are clear from a distance. |
| 6 (A-) | Clean and readable throughout. One slide is slightly dense or one chart could be clearer, but overall professional. |
...
| 4 (B) | Readable but unpolished. Mix of strong and weak slides — some clean, others text-heavy or visually cluttered. |
| 3 (B-) | Rough overall. Multiple text-heavy slides, inconsistent fonts or formatting, charts without clear takeaways. |
| 2 (C) | Hard to follow visually. Walls of text, cluttered layouts, or design that distracts from the content. |
...
> **Deck Quality Score** = (Storyline × 0.4 + Analysis × 0.4 + Craft × 0.2) × 50 ÷ 7
